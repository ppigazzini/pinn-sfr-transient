...
* float64 throughout; hard initial conditions.
...
class Trainer:
    ...
    def causal_loss(self, t: torch.Tensor) -> torch.Tensor:
        ...
        # Segmented mean, not a Python loop over masks: `bool(mask.any())` and
        # boolean-mask indexing both need the mask on the host, so the loop cost
        # two GPU->CPU synchronisations per chunk and serialised the step
        # against the device. `index_add` keeps the whole reduction on device.
        # An empty chunk divides 0 by the clamped count and contributes 0 to the
        # loss and 0 to the gradient, exactly as `e.sum() * 0.0` did.
        z = torch.zeros(self.cfg.causal_chunks, dtype=e.dtype, device=e.device)
        counts = z.index_add(0, idx, torch.ones_like(e))
        L = z.index_add(0, idx, e) / counts.clamp(min=1.0)  # (M,)
